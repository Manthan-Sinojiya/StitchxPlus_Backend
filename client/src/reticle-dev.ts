...
if (import.meta.env.DEV) {
  ...
  registerStore('useapp', useAppStore);
  registerStore('useauth', useAuthStore);
  registerStore('usecart', useCartStore);

  registerCapabilities({
    testids: ['thumbnail-${idx}', 'main-image-container', 'main-image', 'lightbox-modal', 'lightbox-close'],
    signals: [], // names you pass to reticle.signal()
    stores: ['useapp', 'useauth', 'usecart'], // the keys you registered above
  });
}
